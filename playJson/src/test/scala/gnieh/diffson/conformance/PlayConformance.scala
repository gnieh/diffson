...
class PlayConformance extends TestRfcConformance[JsValue, PlayJsonInstance](playJson) {
  ...
  implicit lazy val conformanceTestUnmarshaller: Reads[ConformanceTest] = Reads {
    ...
    case json =>
      throw new FormatException(f"Test record expected but got ${Json.stringify(json)}")
  }
